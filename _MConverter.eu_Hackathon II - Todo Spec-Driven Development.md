...
1.  [[Is Spec-Driven Development Key for Infrastructure Automation?]{.underline}](https://thenewstack.io/is-spec-driven-development-key-for-infrastructure-automation/)

2.  [[ChatGPT Progressive Learning Conversation]{.underline}](https://chatgpt.com/share/6924914a-43dc-8001-8f67-af29c4d9617e)

3.  [[Spec-Driven Cloud-Native Architecture: Governing AI Agents for Managed Services with Claude Code and SpecKit]{.underline}](https://claude.ai/public/artifacts/6025a232-6ebe-4c42-bb51-02dbd4603e18)
...
## Intermediate Level (Organization & Usability) {#intermediate-level-organization-usability}
...
[[https://forms.gle/KMKEKaFUD6ZX4UtY8]{.underline}](https://forms.gle/KMKEKaFUD6ZX4UtY8)  
...
3.  Include a demo video link (must be under 90 seconds). Judges will only watch the first 90 seconds. You can [[use NotebookLM]{.underline}](https://www.youtube.com/watch?v=_9TgVAYP3XA) or record your demo.
...
- [[https://us06web.zoom.us/j/84976847088?pwd=Z7t7NaeXwVmmR5fysCv7NiMbfbhIda.1]{.underline}](https://us06web.zoom.us/j/84976847088?pwd=Z7t7NaeXwVmmR5fysCv7NiMbfbhIda.1)
...
💡**Development Approach:** Use the [[Agentic Dev Stack workflow]{.underline}](#the-agentic-dev-stack-agents.md-spec-kitplus-claude-code): Write spec → Generate plan → Break into tasks → Implement via Claude Code. No manual coding allowed. We will review the process, prompts, and iterations to judge each phase and project.
...
## /specs/overview.md {#specsoverview.md}
...
## /specs/features/task-crud.md {#specsfeaturestask-crud.md}
...
## /specs/api/rest-endpoints.md {#specsapirest-endpoints.md}
...
## /specs/database/schema.md {#specsdatabaseschema.md}
...
# Workflow with Spec-KitPlus + Claude Code {#workflow-with-spec-kitplus-claude-code}
...
# OpenAI ChatKit Setup & Deployment {#openai-chatkit-setup-deployment}
...
- Navigate to: [[https://platform.openai.com/settings/organization/security/domain-allowlist]{.underline}](https://platform.openai.com/settings/organization/security/domain-allowlist)
...
Use [[Docker AI Agent (Gordon)]{.underline}](https://docs.docker.com/ai/gordon/) for intelligent Docker operations:
...
Use [[kubectl-ai]{.underline}](https://github.com/GoogleCloudPlatform/kubectl-ai), and [[Kagent]{.underline}](https://github.com/kagent-dev/kagent) for intelligent Kubernetes operations:
...
> Sign up at [[https://azure.microsoft.com/en-us/free/.%22]{.underline}](https://azure.microsoft.com/en-us/free/.%22)?
...
> Sign up at [[https://cloud.google.com/free?hl=en]{.underline}](https://cloud.google.com/free?hl=en)

Kafka Use Cases in Phase

**Event-Driven Architecture for Todo Chatbot**

# 1. Reminder/Notification System {#remindernotification-system}
...
# 2. Recurring Task Engine {#recurring-task-engine}
...
# 3. Activity/Audit Log {#activityaudit-log}
...
# 4. Real-time Sync Across Clients {#real-time-sync-across-clients}
...
# Use Case 3: Service Invocation (Frontend → Backend) {#use-case-3-service-invocation-frontend-backend}
...
## 1. The Mental Model: Who Does What? {#the-mental-model-who-does-what}
...
## 2. Step 1: Initialize Spec-KitPlus {#step-1-initialize-spec-kitplus}
...
## 3. Step 2: Create a Spec-Aware AGENTS.md {#step-2-create-a-spec-aware-agents.md}
...
## 4. Step 3: Wire Spec-KitPlus into Claude via MCP {#step-3-wire-spec-kitplus-into-claude-via-mcp}
...
### 4.1 Install SpecKitPlus, Create an MCP Server {#install-speckitplus-create-an-mcp-server}
...
### 4.2 Register with Claude Code {#register-with-claude-code}
...
## 5. Step 4: Connect Claude Code via the \"Shim\" {#step-4-connect-claude-code-via-the-shim}

Copy the default [[CLAUDE.md]{.underline}](http://claude.md) file and integrate the content within AGENTS.md . Claude Code automatically looks for CLAUDE.md. To keep a single source of truth, use a redirection pattern.
...
## 6. Step 5: The Day-to-Day Workflow {#step-5-the-day-to-day-workflow}
...
## 7. Constitution vs. AGENTS.md: The Difference {#constitution-vs.-agents.md-the-difference}
